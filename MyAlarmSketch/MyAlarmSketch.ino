...
#define HOUROFFSETINDEX 0
#define MINOFFSETINDEX 1
...
class homeScreen {
  ...
  int hourOffset = EEPROM.read(HOUROFFSETINDEX), minuteOffset = EEPROM.read(MINOFFSETINDEX);
  ...
  //increases the minOffset by one, starts at 0 if over 59
  private: void increaseMinOffset() {
      if ( (minuteOffset + 1) > 59) { minuteOffset = 0; }
      else { minuteOffset++; }
  ...
  //decreases the minOffset by one, starts at 60 if under 0
  private: void decreaseMinOffset() {
      if ( (minuteOffset - 1) < 0) { minuteOffset = 59; }
      else { minuteOffset--; }
  ...
  //increases the hourOffset by one, starts at 0 if over 23
  private: void increaseHourOffset() {
      if ( (hourOffset + 1) > 23) { hourOffset = 0; }
      else { hourOffset++; }
  ...
  //decreases the hourOffset by one, starts at 24 if under 0
  private: void decreaseHourOffset() {
      if ( (hourOffset - 1) < 0) { hourOffset = 23; }
      else { hourOffset--; }
 ...
 public: void drawScreen(){
  ...
  //overdraws ONLY the hour
  public: void updateHour(int hour_) { showTime(hourDim[0], hourDim[1], (hour_ + hourOffset) % 24); }

  //overdraws ONLY the minute
  public: void updateMin(int min_) { showTime(minDim[0], minDim[1], (min_ + minuteOffset) % 60); }


  //overload the ++ and the -- operator for easier time manipulation
...
class alarmScreen : public setTimeScreen {
  ...
  bool wakeMeUp() {
    ...
    Serial.print( ((t.hour + EEPROM.read(HOUROFFSETINDEX)) % 24) );
    Serial.print(" ");
    Serial.print( ((t.min + EEPROM.read(MINOFFSETINDEX)) % 60) );
    Serial.println("");
    ...
    if(((t.hour + EEPROM.read(HOUROFFSETINDEX)) % 24) == EEPROM.read(ALARMHOURINDEX) && ((t.min + EEPROM.read(MINOFFSETINDEX)) % 60) == EEPROM.read(ALARMMININDEX) && wakeActive){
...
class slumberScreen : public setTimeScreen {
  ...
  void slumber(alarmScreen &myAlarmScreen){
    ...
    if(((t.hour + EEPROM.read(HOUROFFSETINDEX)) % 24) == EEPROM.read(SLUMBERHOURINDEX) && ((t.min + EEPROM.read(MINOFFSETINDEX)) % 60) == EEPROM.read(SLUMBERMININDEX) && slumberActive){
